...
function Desert() {
  ...
  const [timer, setTimer] = useState(300); // タイマーを60秒に設定
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.REACT_APP_API_URL}/api/v1/images`)
      ...
      .then(data => {
        ...
        const desertImages = data.filter(image => image.name.includes('砂漠'));
        setImages(desertImages);
        setIsLoading(false);
      })
      .catch(error => {
  ...
  if (!images.length) return <div>砂漠の画像がありません。</div>;
  ...
  return (
    <div>
      ...
      {selectedImage && (
        <div>
          <img src={selectedImage.direct_link} alt={selectedImage.name} draggable="false"/>
